...
func build_circle_rot(pos, segments, radius = 1, rotation = 0):
	var circle = []
	circle.resize(segments + 1)
	
	var s_angle = Utils.TWO_PI/segments
	
	var matrix = Basis(Vector3(0, 1, 0), rotation)
...
func update():
	...
	for i in range(torus_segments):
		var m1 = Basis(Vector3(0, 1, 0), angle * i)
		
		if slice_to or i < torus_segments - 1:
			var m2 = Basis(Vector3(0, 1, 0), angle * (i + 1))
	...
	if generate_ends and slice_to > 0:
		var m = Basis(Vector3(0, 1, 0), slice_angle)
