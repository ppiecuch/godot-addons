...
const SELF_PATH: String = "res://addons/_autoload/globals/class_type.gd"
...
static func static_is_object_instance_of(p_object, p_type, p_map: Dictionary = {}) -> bool:
	...
	var script_map = p_map
	if script_map.empty():
		script_map = _get_script_map()
	if node and node.filename:
		return static_is_type(load(node.filename), p_type, script_map)
	var script := p_object.get_script() as Script
	if script:
		return static_is_type(script, p_type, script_map)
	return static_is_type(p_object.get_class(), p_type, script_map)
...
static func static_is_type(p_type, p_other, p_map: Dictionary = {}) -> bool:
	...
	match typeof(p_type):
		...
		TYPE_STRING:
			...
			var res_type := _convert_name_to_res(p_type, script_map)
			if res_type:
				return static_is_type(res_type, p_other, script_map)
		...
		TYPE_OBJECT:
			...
			match typeof(p_other):
				...
				TYPE_STRING:
					...
					var res_other := _convert_name_to_res(p_other, script_map)
					if res_other:
						return static_is_type(p_type, res_other, script_map)
